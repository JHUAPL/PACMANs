function [timeUTC,depth,meanlatTA,meanlonTA,volumeTA,meanlatVA,meanlonVA,volumeVA,meanVA,meanTA,meanSA,meanrho0A,D0A,meanlonTP,meanlatTP,volumeTP,meanlonVP,meanlatVP,volumeVP,meanVP,meanTP,meanSP,meanrho0P,D0P] = zonalVTSrho0LElowmemory(fileV,fileT,fileS)
...
lat_degN_1 = ncread(fileS,'TLAT'); % nlon,nlat 320x384
lon_degE_1 = ncread(fileS,'TLONG'); % nlon,nlat 320x384
depth = ncread(fileS,'z_t')/100; % depth (m)
[ny,nx]=size(lat_degN_1);
nz=length(depth);
lat_degN_1v = ncread(fileV,'ULAT'); % nlon,nlat 320x384
lon_degE_1v = ncread(fileV,'ULONG'); % nlon,nlat 320x384
...
regions=ncread(fileS,'REGION_MASK');
tarea=ncread(fileS,'TAREA')./1e4;%cm^2-->m^2
uarea=ncread(fileS,'UAREA')./1e4;
...
for ii=1:ny
    ...
    hold_SAL = squeeze(ncread(fileS,'SALT',start,count,stride)); % (lon, lat, depth, monthly time) 320x1x1x600
    hold_T = squeeze(ncread(fileT,'TEMP',start,count,stride));
    hold_V = squeeze(ncread(fileV,'VVEL',start,count,stride))./100;%cm/s-->m/s
    ...
    hold_V=squeeze(hold_V_all(:,jj,:));
        ...
        volumeVA(ii,jj)=nansum(dz(jj)*uarea(iwantv,ii));
        meanVA(ii,jj,:)=nansum(hold_V(iwantv,:)*dz(jj).*repmat(uarea(iwantv,ii),1,count(4)),1)./volumeVA(ii,jj);
        ...
        %density
        P = gsw_p_from_z(-depth(jj)*ones(sum(iwant),1),lat_degN_1(iwant,ii)); % pressure(decibars); 1 decibar = 1 meter of seawater
...
%         meanrho2k(ii,jj,:)=nansum(rho2k*dz(jj).*repmat(tarea(iwant,ii),1,count(4)),1)./volume1(ii,jj); %potential density
        ...
        volumeVP(ii,jj)=nansum(dz(jj)*uarea(iwantv,ii));
        meanVP(ii,jj,:)=nansum(hold_V(iwantv,:)*dz(jj).*repmat(tarea(iwantv,ii),1,count(4)),1)./volumeVP(ii,jj);
