...
def last_point_aredi(box_dims, box_init, box_params, box_ts, box_rand):
    """
    ...
    """
    M_n, M_u, M_eddy, Dlow, T, S, sigma0 = box_model(box_dims, box_init, box_params, box_ts, box_rand).unpack()
    return M_n[-1], M_u[-1], M_eddy[-1], sigma0[0, -1] - sigma0[2, -1], Dlow[-1]


def fba_run_n_steps(n_steps, box_dims, box_init, box_params, box_ts, box_rand):
...
def fba_run_n_steps_async(n_steps, box_dims, box_init, box_params, box_ts, box_rand, poolsize=12):
    Mnsave = np.zeros((n_steps, 1))
    Musave = np.zeros((n_steps, 1))
    Meddysave = np.zeros((n_steps, 1))
    dsigmasave = np.zeros((n_steps, 1))
    ...
    for Fwn in np.arange(n_steps) * 0.05e6:
        new_box_params = box_params.copy()
        new_box_params.Fwn = Fwn
        async_args.append((box_dims, box_init, new_box_params, box_ts, box_rand))
