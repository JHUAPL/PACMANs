...
"""
This script simulates a search over the box model parameter space for parameter sets where the AMOC collapses.
For each box model run, a single parameter is chosen at random and altered by a random amount relative to the size of
the parameter's current value. The box model data is generated and stored in NetCDF format. Each run holds a set of
pointers to newer runs that were generated by perturbing one of that run's parameters, creating a tree like structure
between all runs that are generated.
"""
...
def main(netcdf_fp):
    ...
    for i in range(10):
        ...
        for idx in sample_indices:
            ...
            run_and_save_box_run(box_args_from_dict(new_params), netcdf_fp, run_id, base_id)
            iterations += 1
