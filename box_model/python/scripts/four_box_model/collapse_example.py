...
def sim1(num_procs=12):
    ...
    randomization = BoxModelRandomization()  # defaults to no randomness

    box_params.M_ek = 15e6
    out = fba_run_n_steps(n_steps, box_dims, init_conditions, box_params, time_step, randomization) if num_procs == 1 \
        else fba_run_n_steps_async(n_steps, box_dims, init_conditions, box_params, time_step,
                                   randomization, poolsize=num_procs)
    Mnsave15, Musave15, Meddysave15, dsigmasave15, Dlowsave15 = out

    box_params.M_ek = 25e6
    out = fba_run_n_steps(n_steps, box_dims, init_conditions, box_params, time_step, randomization) if num_procs == 1 \
        else fba_run_n_steps_async(n_steps, box_dims, init_conditions, box_params, time_step, randomization,
                                   poolsize=num_procs)
